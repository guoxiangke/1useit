...
class auth_plugin_drupal extends auth_plugin_base {
  ...
  function loginpage_hook() {
    ...
    $drupal_user = $DB->get_record_sql("SELECT u.* FROM {$this->drupal_db_prefix}users u
      LEFT JOIN {$this->drupal_db_prefix}sessions s on (s.uid = u.uid)
  ...
  function sync_users($do_updates = FALSE) {
    ...
    if (!empty($this->config->removeuser)) {
      ...
      $remove_users = $DB->get_records_sql("SELECT u.id, u.username, u.email, u.auth " .
        ...
        "(SELECT name COLLATE utf8_unicode_ci FROM {$this->drupal_db_prefix}users users)"
      );
    ...
    if (count($users_to_sync)) {
      foreach ($users_to_sync as $username) {
        ...
        echo "\n";
        $user = $DB->get_record_sql("SELECT users.* FROM {$this->drupal_db_prefix}users users " .
  ...
  function user_exists($username) {
    return $DB->record_exists_sql("SELECT uid FROM {$this->drupal_db_prefix}users users WHERE name = '$username'");
  }
  ...
  function get_userlist() {
    ...
    $result = array();
    // fetch userlist
    $user_names = $DB->get_recordset_sql("SELECT name AS username FROM {$this->drupal_db_prefix}users users WHERE uid > 0");
