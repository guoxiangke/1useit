...
function delete_user(stdClass $user) {
      if( $drupal_dbh = drupal_db_conn() ){
		  global $DRUPAL_DB;
        $d_sql = 'DELETE FROM '.$DRUPAL_DB['default']['default']['prefix']."users WHERE name='".$user->username."'" ;
...
function update_internal_user_password($user, $password) {
    ...
    if ($passwordchanged || $algorithmchanged) {
        if( $drupal_dbh = drupal_db_conn() ){
          global $DRUPAL_DB;
          $drupal_pass = drupal_user_hash_password($password);
          $d_sql = 'UPDATE '.$DRUPAL_DB['default']['default']['prefix']."users SET pass='$drupal_pass' WHERE name='".$user->username."'" ;
          mysql_query( $d_sql,$drupal_dbh );
        }
        else{
          die('Can not connect to drupal database! '.__FUNCTION__.' ' .__LINE__);
...
function drupal_db_conn(){
	include('config4drupal2.php');
	include('../config4drupal2.php');
	include('../../config4drupal2.php');
	include('../../../config4drupal2.php');
	include('../../../../config4drupal2.php');
	$resource = mysql_connect($DRUPAL_DB['default']['default']['host'].($DRUPAL_DB['default']['default']['port']?':'.$DRUPAL_DB['default']['default']['port']:''), $DRUPAL_DB['default']['default']['username'], $DRUPAL_DB['default']['default']['password'], true) or die(mysql_error());
	mysql_select_db($DRUPAL_DB['default']['default']['database'],$resource) or die ('Can\'t use drupl DB : ' . mysql_error());;
	return $resource;
}
 ...
 */
function drupal_user_hash_password($password, $count_log2 = 0) {
  if (empty($count_log2)) {
    // Use the standard iteration count.
    $count_log2 = DRUPAL_HASH_COUNT;
  }
  return _password_crypt2('sha512', $password, _password_generate_salt2($count_log2));
}
 ...
 */
function drupal_random_bytes2($count)  {
 ...
 *   Encoded string
 */
function _password_base64_encode2($input, $count) {
  $output = '';
  $i = 0;
  $itoa64 = _password_itoa642();
  do {
    $value = ord($input[$i++]);
 ...
 */
function _password_generate_salt2($count_log2) {
  $output = '$S$';
  // Ensure that $count_log2 is within set bounds.
  $count_log2 = _password_enforce_log2_boundaries2($count_log2);
  // We encode the final log2 iteration count in base 64.
  $itoa64 = _password_itoa642();
  $output .= $itoa64[$count_log2];
  // 6 bytes is the standard salt for a portable phpass hash.
  $output .= _password_base64_encode2(drupal_random_bytes2(6), 6);
  return $output;
}
 ...
 */
function _password_enforce_log2_boundaries2($count_log2) {
 ...
 */
function _password_crypt2($algo, $password, $setting) {
  ...
  $count_log2 = _password_get_count_log22($setting);
  ...
  $len = strlen($hash);
  $output =  $setting . _password_base64_encode2($hash, $len);
 ...
 */
function _password_get_count_log22($setting) {
  $itoa64 = _password_itoa642();
