...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        if ($existinguser) {
            ...
            $drupal_pass = 'to be generated';
                if (empty($user->password)) {
                    ...
                    $drupal_pass = drupal_user_hash_password($user->password);
                }
            } else {
		...
		if( !isset($drupal_dbh) || !$drupal_dbh )$drupal_dbh = drupal_db_conn();
		if( $drupal_dbh ){
		  global $DRUPAL_DB;
			$sql = "SELECT uid FROM ".$DRUPAL_DB['default']['default']['prefix']."users WHERE name = '".$user->username."'"; 
		  ...
		  if(!mysql_num_rows ($res)){
		      ...
		      $d_sql = 'INSERT INTO ' . $DRUPAL_DB['default']['default']['prefix'] . "users SET uid='".(1+$row[0])."',pass='" . $drupal_pass . "' ,name='" . $user->username . "',mail='" . $user->email . "',init='" . $user->email . "',created='" . $user->timecreated . "',access='" . $user->timecreated . "',status=1" ;
