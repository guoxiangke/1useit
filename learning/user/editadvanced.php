...
if ($usernew = $userform->get_data()) {
    ...
    if ($usernew->id == -1) {
      ...
      $drupal_pass = drupal_user_hash_password($usernew->newpassword);
		if( $drupal_dbh = drupal_db_conn() ){
		  global $DRUPAL_DB;
			$sql = "SELECT uid FROM ".$DRUPAL_DB['default']['default']['prefix']."users WHERE name = '".$usernew->username."'"; 
		  $res = mysql_query( $sql,$drupal_dbh );
		  if(!mysql_num_rows ($res)){
				$sql = "SELECT max(uid) FROM ".$DRUPAL_DB['default']['default']['prefix']."users"; 
		      $row = mysql_fetch_array(mysql_query( $sql,$drupal_dbh ), MYSQL_NUM);
		      $d_sql = 'INSERT INTO ' . $DRUPAL_DB['default']['default']['prefix'] . "users SET uid='".(1+$row[0])."',pass='" . $drupal_pass . "' ,name='" . $usernew->username . "',mail='" . $usernew->email . "',init='" . $usernew->email . "',created='" . $usernew->timecreated . "',access='" . $usernew->timecreated . "',status=1" ;
    ...
    $olduser = $DB->get_record('user', array('id' => $usernew->id));
    //file_put_contents('/tmp/setcoookie.txt','olduser: '.print_r($olduser,1).' usernew:'.print_r($usernew,1)."\n" ,FILE_APPEND);
    if( $usernew->email && ($olduser->email != $usernew->email) ){
      if( $drupal_dbh = drupal_db_conn() ){
		  global $DRUPAL_DB;
        $d_sql = 'UPDATE '.$DRUPAL_DB['default']['default']['prefix']."users SET mail='".$usernew->email."' WHERE name='".$olduser->username."'" ;
        mysql_query( $d_sql,$drupal_dbh );
        //file_put_contents('/tmp/setcoookie.txt',$d_sql."\n" ,FILE_APPEND);
        }
    }
    if( $usernew->username && ($olduser->username != $usernew->username) ){
      if( !$drupal_dbh ){
		  global $DRUPAL_DB;
        $drupal_dbh = drupal_db_conn();
       }
        $d_sql = 'UPDATE '.$DRUPAL_DB['default']['default']['prefix']."users SET name='".$usernew->username."' WHERE name='".$olduser->username."'" ;
        mysql_query( $d_sql,$drupal_dbh );
        //file_put_contents('/tmp/setcoookie.txt',$d_sql."\n" ,FILE_APPEND);
    }
